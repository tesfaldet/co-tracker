...
class PointOdysseySubSeq(Dataset):
    ...
    def __init__(
        ...
        """Initialize a `PointOdyssey` dataset.
        ...
        means no skipping between images in the sequence, while a frameskip of `1` means every other image is skipped,
        and so on. Default is `[0, 1, 3]`.
        ...
        :raises ValueError: If an invalid `split` is provided. Must be one of `["train", "test", "val"]`.
        """
    ...
    @property
    def samples(self) -> List[Tuple[List[str], List[str], List[str], List[str], List[int], str]]:
        """Get the samples in the dataset.

        :return: The samples in the dataset.
        """
        return self.sampled_sequences[self.split]
    ...
    def __getitem__(self, ind: int) -> Dict[str, Any]:
        ...
        """
        rgb_paths, traj_2d_paths, valid_paths, visib_paths, path_inds, seq_name = self.samples[ind]
        ...
        assert (
            ...
            and valids.shape[0] == self.sequence_length
        ), f"Expected sequence length of {self.sequence_length}, got the following: rgbs: {len(rgbs)}, trajs: {trajs.shape[0]}, visibs: {visibs.shape[0]}, valids: {valids.shape[0]}."
        ...
        # Update visibility and validity annotations, post augmentations. Do the following:
        #   1) Set points along the 1px edge of the image or out of bounds (OOB) to be invisible.
        #   2) Set points that are very OOB (past a 64px buffer zone) to be invalid.
    ...
    def _make_paths(
        ...
        Dict[str, List[Tuple[List[str], List[str], List[str], List[str], List[int], str]]],
    ]:
        """Create dicts containing relevant filepaths for the train, val, and test sets.

        `full_sequences` is a dict containing the full image sequence paths, traj_2d paths, valid paths, and visibs
        paths for the train, val, and test sets.
        ...
        `sampled_sequences` is a dict containing image sequences sampled from `full_sequences` according to
        `self.sequence_length` and `self.frameskips`, for the train, val, and test sets. Each sampled sequence is a
        tuple containing the image paths for the sequence, the trajs_2d paths, the valids paths, the visibs paths,
        indices describing each image path's indexed location within the full image sequence, and the sequence name.
        ...
        full_sequences = {
            "train": {
                seq.name: {
                    "rgb_paths": sorted(
                        [str(rgb_path) for rgb_path in (seq / "rgbs").iterdir() if self._is_valid_file(str(rgb_path))]
                    ),
                    ...
                    "valid_paths": sorted(
                        [
                            str(valid_path)
                            for valid_path in (seq / "valids").iterdir()
        ...
        sampled_sequences = {
            "train": [],  # ([rgb_paths], [traj_2d_paths], [valid_paths], [visib_paths], [inds], seq_name)
            "test": [],  # ([rgb_paths], [traj_2d_paths], [valid_paths], [visib_paths], [inds], seq_name)
            "val": [],  # ([rgb_paths], [traj_2d_paths], [valid_paths], [visib_paths], [inds], seq_name)
        }

        # Build sampled sequences for the train set.
        for seq_name, seq_paths in full_sequences["train"].items():
        ...
        # Build sampled sequences for the test set.
        for seq_name, seq_paths in full_sequences["test"].items():
            full_seq_len = len(seq_paths["rgb_paths"])
            if self.sequence_length > full_seq_len:
                continue
            for skip in self.frameskips:
        ...
        # Build sampled sequences for the val set.
        for seq_name, seq_paths in full_sequences["val"].items():
            full_seq_len = len(seq_paths["rgb_paths"])
            if self.sequence_length > full_seq_len:
                continue
            for skip in self.frameskips:
